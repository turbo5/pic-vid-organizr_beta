...
modifyed by turbo:
http://www.janeriksolem.net/2009/02/sift-python-implementation.html - comments
plus other things
"""
...
def process_image(imagename, resultname):
    ...
    try:
        os.system(cmmd)
        print 'processing', imagename, 'finished\n'
        return True
    except: return False
...
def match(desc1,desc2):
    ...
    for i in range(desc1_size[0]):        
        ...
        if len(dotprods) == 0:
            ...
            # print arccos(dotprods)[indx[0]], ' '
            # print 'less than this?: ', dist_ratio * arccos(dotprods)[indx[1]]        
...
def plot_features(im,locs):
    ...
    color = 'o'+str(color)
    pylab.plot([p[1] for p in locs], [p[0] for p in locs], color)
    pylab.axis('off')
    pylab.show()
